...
*/
(function() {
  app.controller("QuotationController", ["$scope","$state","$ionicPlatform","Calculation","StorageUserModel","Motors","$ionicModal","popUpService","$resource","translationService","Quotation","$cordovaStatusbar","Utils",
    ...
    $ionicPlatform.ready(function() {
      ...
      if (window.cordova) {
        ...
        $scope.checkFile = function(_file_name, _url) {
          ...
          $cordovaFile.checkFile(targetPath, file_path).then(
            function(success) {
        ...
        $scope.downloadFile = function(_url, _file_name) {
          ...
          $cordovaFileTransfer.download(_url, path, options, trustHosts).then(
            function(result) {
              console.log(result);
            },
            function(err) {
